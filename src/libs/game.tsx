...
**備註** 遊戲過程中可獲得特殊模式，詳見其他模式說明。`,
  ...
  greenwei: {
    name: "綠光阿瑋模式",
    detail: `**觸發** 當輪出現三個「\`$greenweiVar.bindPattern.name$\`」有機率觸發或累積次數達時。
  - 機率 $greenweiVar.rate$%
...
**結束** 綠光阿瑋次數歸零時自動退出模式。`,
  ...
  pikachu: {
    name: "皮卡丘充電",
    detail: `**觸發時機** 遊玩次數用盡時出現任一「\`$pikachuVar.bindPattern.name$\`」。
...
**結束** 當額外次數用盡且該輪未再出現 「\`$pikachuVar.bindPattern.name$\`」 時結束遊戲。`,
  ...
  superhhh: {
    name: "超級阿禾模式",
    detail: `**觸發** 當輪出現任意「\`$superhhhVar.bindPattern.name$\`」且有機率觸發。
  - 機率 \`$superhhhVar.rate$\`%
