...
- **結束條件**：當可玩次數用盡，顯示最終成績。
  ...
  greenwei: {
    name: "綠光阿瑋模式",
    detail: `
- **觸發**：當輪出現三個「$greenweiVar.bindPattern.name$」有機率觸發或累積次數達標時。
  - 機率：$greenweiVar.rate$%；累積次數達 $greenweiVar.requiredBindPatternCount$ 時自動觸發。
- **回合數**：觸發時獲得基礎回合，期間可因條件增加回合。
  - 啟動獲得 +$greenweiVar.bonusTimes$ 回合；期間同時出現全「$greenweiVar.bindPattern.name$」再 +$greenweiVar.extendTimes$
...
- **觸發**：當輪出現任意「$superhhhVar.bindPattern.name$」且有機率觸發。
  - 機率：$superhhhVar.rate$%；初次啟動獲得 $superhhhVar.bonusTimes$ 回合。
- **回合數**：觸發時獲得基礎回合，期間可因條件增加回合。
